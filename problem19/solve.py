...
def cut(leaderboard, spectrum, n):
    ...
    for peptide in leaderboard:
        linear_score_dict[peptide] = linear_score(peptide, spectrum)
...
def score(peptide, spectrum):
    ...
    linear_spectrum = cyclospectrum(petpide_list)
    score = 0
    for key, value in linear_spectrum.items():
        v = spectrum.get(key, 0)
        if v >= value:
...
def solve_problem():
    rosalind_input = "rosalind_ba4g.txt"
    rosalind_output = "rosalind_ba4g_output.txt"
