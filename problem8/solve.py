...
def random_solve(dna, k, t):
    best_motifs = []
    for i in range(len(dna)):
        pos = int(random.uniform(0.0, len(dna[0])-k))
        pos = pos if pos < len(dna[0])-k-1 else pos-1
...
def solve(dna, k, t):
    best_motifs, best_score = random_solve(dna, k, t)
    for _ in range(1000):
        motifs, score = random_solve(dna, k, t)
        if score < best_score:
            best_motifs = motifs[:]
            best_score = score
...
def solve_problem():
    rosalind_input = "rosalind_ba2f.txt"
    rosalind_output = "rosalind_ba2f_output.txt"
